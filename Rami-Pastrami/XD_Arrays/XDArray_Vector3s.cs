...
public class XDArray_Vector3s : UdonSharpBehaviour
{
    ...
    /// <summary>
    /// Converts normal arrays to XDV3Arrays.
    /// for dimensions, do [(length dimension 1, length dimension 2 , ... , length dimension n)] 
    /// </summary>
    /// <param name="input"></param>
    /// <param name="dimensions"></param>
    /// <returns></returns>
    public Vector3[] ConvertToXDV3Arr(Vector3[] input, int[] dimensions)
    {
        int numDims = dimensions.Length;
        int startIndex = numDims + 1;
        Vector3[] output = new Vector3[(input.Length + numDims + 1)];
    ...
    /// <summary>
    /// Helper function for above, converts a 1D Vector3s array to a 1D XDV3Array
    /// </summary>
    /// <param name="input"></param>
    /// <returns></returns>
    public Vector3[] ConvertTo1DV3Arr(Vector3[] input)
    {
        int[] dimensions = new int[] { input.Length };
        return ConvertToXDV3Arr(input, dimensions);
    }

    /// <summary>
    /// Converts XDV3Array into a normal Vector3 array by stripping header.
    /// WARNING: ALL Dimension information will be lost!
    /// </summary>
    /// <param name="XDV3Arr"></param>
    /// <returns></returns>
    public Vector3[] ConvertToV3Arr(Vector3[] XDV3Arr)
    {
        ...
        {
            output[(i - offset)] = XDV3Arr[i];
        }
        return output;
    }

    /// <summary>
    /// Create a zero'd XDV3Array of given dimensions
    /// </summary>
    /// <param name="dimensions"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// returns dimension information from XDV3Array
    /// </summary>
    /// <param name="input"></param>
    /// <returns></returns>
    ...
    /// <param name="indexOffset"></param>
    /// <returns></returns>
    public Vector3 ReadSingleElementFromXDV3Arr_OPT(Vector3[] readFrom, int[] coords, int[] kick, int indexOffset)
    {
    ...
    /// <summary>
    /// read array segment of XDV3Array into another XD{L}}Array of the same number of dimensions
    /// </summary>
    /// <param name="readFrom"></param>
    /// <param name="readingStartCoords"></param>
    /// <param name="distFromCords"></param>
    /// <returns></returns>
    public Vector3[] ReadXDV3ArrFromXDV3Arr(Vector3[] readFrom, int[] readingStartCoords, int[] distFromCords)
    {
        int[] readingDims = ReadXDV3ArrDims(readFrom); //input num dims = output num dims
        Vector3[] readTo = CreateXDV3Arr(distFromCords); //XDArray that the read data will be written to and outputted!
    ...
    /// <summary>
    /// Loop-Optimized version of WriteSingleElementToXDV3Array, follow instructions of what variables to pass.
    /// This is done to avoid repetitive calculations
    /// </summary>
    /// <param name="writeTo"></param>
    /// <param name="coords"></param>
    ...
    /// <param name="indexOffset"></param>
    /// <returns></returns>
    public Vector3[] WriteSingleElementToXDV3Array_OPT(Vector3[] writeTo, int[] coords, Vector3 ValueToWrite, int[] kick, int indexOffset)
    {
        //calculate the following following this method. Not included in function for optimization reasons
        //but whatever is calling this function can get this functions inputs with the following methods
        //int[] dims = ReadXDV3ArrDims(readFrom); //for below!
        //int[] kick = CalcPerCoordKick(dims);
        //int indexOffset = 1 + coords.Length;
        writeTo[GetRawIndexFromCoordsFull(coords, kick, indexOffset)] = ValueToWrite;
        return writeTo;
    ...
    /// <summary>
    /// overwrites set values of target XDV3Array with source XDV3Array starting from targetStartCoords
    /// </summary>
    /// <param name="target"></param>
    /// <param name="targetStartCoords"></param>
    /// <param name="source"></param>
    /// <returns></returns>
    public Vector3[] WriteXDV3ArrToXDV3Arr(Vector3[] target, int[] targetStartCoords, Vector3[] source)
    {
        int[] targetDims = ReadXDV3ArrDims(target); //input num dims = output num dims
        int[] sourceDims = ReadXDV3ArrDims(source); //these dimensions should be smaller than the target (or the same size, but why bother using this then?)
        ...
        int indexOffsetFromHeader = targetDims.Length + 1; //This is the same for all XDV3Arrays involved here!
    ...
    /// <summary>
    /// Appends a new dimension of length 1 to an XDV3Array
    /// </summary>
    /// <param name="input"></param>
    /// <returns></returns>
    public Vector3[] DimensionAddToXDV3Array(Vector3[] input)
    {
    ...
    /// <summary>
    /// Removes a dimension from an XDV3 array.
